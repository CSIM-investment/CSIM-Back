...
export class GoogleNewsApiService {
    ...
    async getFromHoursToNow(
        hours: number,
    ): Promise<GoogleNewsApiResponse<CreateArticle>> {
        this.subject = 'crypto'
        const apiUrl = `${this.getApiUrl()}&from=${this.getDateFromHoursAgo(
            hours,
        )}`
        const { data } = await firstValueFrom(
            this.httpService.get<GoogleNewsApiResponse<GoogleNewsApiArticle>>(
                apiUrl,
            ),
        )
        const articles = data.articles.map((article) =>
            new GoogleNewsApiArticle(article).toCreateArticle(),
        )
